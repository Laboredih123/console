...
/obj/signal/hub/router/process_signal(obj/signal/packet/S, obj/source)
	..()
	if(!S) return
	...
	if (source != src.line_control)
		...
		spawn( 0 )
			...
			if ((src.mode == "normal" || ((nflags & 2 && src.position >= length(L)) || ((src.mode == "5>1" && !( src.line_temp )) || (src.mode == "5>1" && source == src.line_temp)))))
