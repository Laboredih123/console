/mob/var/tmp/obj/items/concealed_wire/con_using
/mob/var/tmp/obj/signal/concealed_wire/p_wire
/mob/var/tmp/viewing_under = FALSE
...
/obj/items/concealed_wire/moved(mob/user,turf/oldloc)
	if(user.con_using)
		var/turf/T = user.loc
		if(user.p_wire)
...
/obj/signal/concealed_wire
			name = "underground wiring terminal"
			icon = 'icons/concealed_wiring.dmi'

			var/tmp/working = FALSE
			var/tmp/obj/signal/line1
			var/tmp/list/connected_wires = list()
			var/tmp/obj/signal/concealed_wire/connected_terminal

/obj/signal/concealed_wire/verb/view_wires()
...
/obj/signal/concealed_wire/verb/label(T as text)
	set src in oview(usr,1)
	if(!T) return
	name = "underground wiring terminal - '[T]'"

/obj/signal/concealed_wire/orient_to(obj/target,mob/user)
	if(ismob(src.loc))
