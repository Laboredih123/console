...
/datum/file/normal/proc/execute(params in view(usr.client), flag in view(usr.client))
	...
	var/r_text
	while((findtext(n_text, "arg[counter]", 1, null) && length(L) >= counter))
		n_text = replacetext(n_text, "arg[counter]", L[counter])
		counter++
	if ((findtext(n_text, "args", 1, null) && length(L) >= counter))
...
/datum/file/normal/executable/copy_to(datum/file/normal/executable/E)
	E.name = src.name
...
/datum/file/normal/executable/dialer/execute(params)
	var/list/t1 = splittext(params, "[ascii2text(2)]")
	if (length(t1) < 2)
