...
/obj/items/toolbox/New()
	new /obj/items/screwdriver( src )
	new /obj/items/wrench( src )
	..()

/obj/items/toolbox/attack_by(obj/items/I, mob/user)
	if (I)
...
/obj/items/toolbox/verb/remove(obj/O in src)
	O.loc = src.loc
...
/obj/boxrack/attack_by(obj/items/I, mob/user)
...
/obj/boxrack/attack_hand(mob/user)
	var/obj/items/box/B = input(user, "Which box would you like to remove?", null, null)  as obj in src
...
/obj/boxrack/verb/remove(obj/O in src)
	set src in view(1)
...
/obj/boxrack/verb/label(msg as text)
	set src in view(1)
...
/obj/filecabinet/attack_by(obj/items/I, mob/user)
	if (I)
		if(istype(I,/obj/items))
			I.unequip()
			user << "\blue You place [I] in [src]."
...
/obj/filecabinet/verb/remove(obj/O in src)
	set src in view(1)

	O.loc = src.loc
	usr << "\blue You remove [O] from [src]."

/obj/filecabinet/verb/label(msg as text)
	set src in view(1)
...
/obj/bulletin/verb/remove(obj/P in src)
	set src in usr.loc
...
/obj/bulletin/attack_hand(mob/user)
	if (src.loc == user.loc)
...
/obj/bulletin/attack_by(obj/items/D, mob/user)
	if (src.loc == user.loc)
