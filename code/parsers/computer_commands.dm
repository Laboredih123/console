/obj/signal/computer/proc/execute(command, params, datum/task/source)
	...
	switch(command)
		...
		if("getenv")
			if(length(command_list) < 1||!command_list[1])
				if(!environment.len)
		...
		/*if("sndsrc")
			if(length(command_list) < 1||length(command_list) > 1)
		...
		if("file_add")
			...
			if (!( t2 ))
				...
				if ((!( istype(t2, /datum/file/normal) ) || t2.flags & 2))
					...
					x = 2
					while(x <= length(command_list))
						msg += "[command_list[x]][(x != length(command_list) ? " " : "")]"
						x++
		...
		if("send")
			...
			if (!( istype(t2, /datum/file) ))
				return
			var/obj/signal/packet/S = new /obj/signal/packet(  )
			...
			if (t2)
				...
				if ((L && length(L) > 1))
					if ((L[1] == "root" || !( L[1] )))
						...
						t = 2
						while(t <= length(L))
							command_list[1] += (t != length(L) ? "[L[t]]/" : "[L[t]]")
							t++
					else
						if (L[1] == "A:")
							if (src.disk)
